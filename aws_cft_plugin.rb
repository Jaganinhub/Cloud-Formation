...
resource_pool "rs_aws_cft" do
  ...
  auth "key", type: "aws" do
    ...
    region     'us-east-1'
    access_key cred('AWS_ACCESS_KEY_ID_J')
    secret_key cred('AWS_SECRET_ACCESS_KEY_J')
  end
end
...
define create_stack(@declaration) return @resource do
  sub on_error: stop_debugging() do
    ...
    call sys_logs.detail(join(["Stack Name: ", $stack_name]))
    ...
    if $status != "CREATE_COMPLETE"
      call sys_logs.detail(@operation.get_stack_events())
      raise "Did not complete provision, check audit entry for details"
    end
    @resource = @operation.get()
    call sys_logs.detail(@resource.get_stack_events())
